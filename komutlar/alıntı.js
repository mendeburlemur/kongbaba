...
exports.run = async (client, message, args, member) => {
  if (!args[0]) return message.channel.send("**Hata:** Lütfen **Bu Kanalda Yazılmış** Bir Mesajın ID'sini Giriniz!")
  if(args[0] < 18) return message.channel.send("**Hata:** Lütfen **Bu Kanalda Yazılmış** Bir Mesajın ID'sini Giriniz!")
  
   let number = ["1","2","3","4","5","6","7","8","9","0"]
    
    if (!number.some(word => message.content.includes(word))) {
           {
                  return message.channel.send("**Hata:** Lütfen **Bu Kanalda Yazılmış** Bir Mesajın ID'sini Giriniz!");
           }}   
...
message.channel.send(embed)
 ...
 if (err.name === 'DiscordAPIError' && err.message === 'Unknown Message') {
       message.channel.send("**Hata:** Lütfen **Bu Kanalda Yazılmış** Bir Mesajın ID'sini Giriniz!"); 
   }
})}
exports.conf = {
  enabled: true,
